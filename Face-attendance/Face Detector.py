...
def validate_student_id(student_id):
    ...
    cursor.execute("SELECT id FROM student_info WHERE student_id = %s", (student_id,))
...
def log_student_entry(student_id):
    ...
    cursor.execute("SELECT last_logged_at FROM student_info WHERE student_id = %s", (student_id,))
    last_logged_at = cursor.fetchone()
    cooldown_period = 5  # Adjust cooldown period in seconds
    last_logged_at_seconds = last_logged_at[0] if last_logged_at else None
    ...
    cursor.execute("INSERT INTO logs (student_id, date, time) VALUES (%s, %s, %s)", (student_id, today, now_str))
    cursor.execute("UPDATE student_info SET last_logged_at = %s WHERE student_id = %s", (now_str, student_id))
    conn.commit()
